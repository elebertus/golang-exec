...
func (s *Script) Command() string {
	...
	case "powershell":
		...
		// Running this from Darwin or linux, os.Getwd() enumerates to localhost's $pwd
		// which obviously isn't assured to exist on the target host. The $HOME variable in powershell expands nicely
		// to the user's home directory so the script ends up in $HOME\_temp-%d.ps1
		...
		return fmt.Sprintf(`
        cmd /E:ON /V:ON /C "more > "%s"
        PowerShell -NoProfile -ExecutionPolicy ByPass -File "%s";
        set E="!errorlevel!";
		del "%s";
        exit $E"`, spath, spath, spath)
	default:
